...
const inputSelector = 'input[ng-combobox-input], input[data-slot="combobox-input"], input[ng-combobox-chip-input], input[data-slot="combobox-chip-input"], input[role="combobox"], input[data-input], input[data-slot="input"], input[ng-input-group-input]';
const itemSelector = '[data-slot="combobox-item"], [ng-combobox-item]';
const rootSelector = '[data-slot="combobox"], [ng-combobox]';
const separatorSelector = '[data-slot="combobox-separator"], [ng-combobox-separator]';
const triggerSelector = '[data-slot="combobox-trigger"], [ng-combobox-trigger]';
const setAttributeIfChanged = (element, name, value) => {
    if (element.getAttribute(name) !== value)
        element.setAttribute(name, value);
};
export function comboboxDirective() {
    return {
        link(scope, element) {
            ...
            const syncChrome = () => {
                ...
                setAttributeIfChanged(content, "data-chips", String(Boolean(owned('[data-slot="combobox-chips"], [ng-combobox-chips]', HTMLElement))));
                if (ownsAriaInvalid) {
            ...
            const positionContent = () => {
                if (!open)
                    return;
                const externalTrigger = ownedAll(triggerSelector).find((trigger) => !content.contains(trigger) && !trigger.closest(anchorSelector));
            ...
            const setOpen = (nextOpen, notifyApplication = false, focusInput = false) => {
                ...
                ownedAll(triggerSelector).forEach((trigger) => {
            ...
            const syncStructure = () => {
                ...
                ownedAll(triggerSelector).forEach((control) => {
                ...
                ownedAll(groupSelector).forEach((group) => {
                    ...
                    const label = queryAll(group, groupLabelSelector).find((candidate) => candidate.closest(groupSelector) === group);
                    if (!label)
                        return;
                    if (!label.id)
