...
const itemSelector = 'a[href], button, [role="menuitem"], [role="menuitemcheckbox"], [role="menuitemradio"]';
export function bindSemanticSubmenus(root, prefix, getDirection) {
    ...
    const bindSubmenu = (submenu) => {
        ...
        const getItems = () => queryAll(content, itemSelector).filter((item) => !isDisabled(item));
        const syncItems = () => {
