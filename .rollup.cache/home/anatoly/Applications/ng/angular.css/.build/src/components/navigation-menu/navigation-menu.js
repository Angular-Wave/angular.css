...
const triggerSelector = '[data-slot="navigation-menu-trigger"], [ng-navigation-menu-trigger]';
const contentSelector = '[data-slot="navigation-menu-content"], [ng-navigation-menu-content]';
const linkSelector = '[data-slot="navigation-menu-link"], [ng-navigation-menu-link]';
const listSelector = '[data-slot="navigation-menu-list"], [ng-navigation-menu-list]';
...
export function navigationMenuDirective() {
    return {
        link(scope, element) {
            ...
            const syncTopLevelControls = () => {
                ...
                const controls = Array.from(list.children).flatMap((child) => {
                    ...
                    const control = directChild(child, triggerSelector) ??
            ...
            const syncStructure = () => {
                boundEntries.forEach((entry, item) => {
                    const replaced = directChild(item, triggerSelector) !== entry.trigger ||
            ...
            const handleKeydown = (event) => {
                ...
                if (!active || !element.contains(active))
                    return;
                const activeContent = active.closest(contentSelector);
                if (event.key === "Escape") {
