...
export function inputOtpDirective() {
    return {
        link(scope, element) {
            ...
            const syncValue = () => {
                ...
                setAttributeIfChanged(element, "data-complete", String(inputs.length > 0 && inputs.every((input) => input.value)));
                setAttributeIfChanged(element, "data-disabled", String(inputs.some((input) => input.disabled)));
                setAttributeIfChanged(element, "data-invalid", String(inputs.some((input) => input.getAttribute("aria-invalid") === "true" ||
            ...
            const bindInput = (input) => {
                ...
                const handleFocus = () => {
                    queryAll(element, '[data-slot="input-otp-slot"], [ng-input-otp-slot]').forEach((slot) => {
                        setAttributeIfChanged(slot, "data-active", String(slot.contains(input)));
                    });
                };
                const handleBlur = () => {
                    const slot = input.closest('[data-slot="input-otp-slot"], [ng-input-otp-slot]');
                    if (slot)
                        setAttributeIfChanged(slot, "data-active", "false");
                };
