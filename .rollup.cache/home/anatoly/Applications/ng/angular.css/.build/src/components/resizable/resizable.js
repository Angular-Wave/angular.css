...
export function resizablePanelGroupDirective() {
    return {
        link(scope, element) {
            ...
            const syncOrientation = () => {
                ...
                setAttributeIfChanged(element, "data-direction", element.closest("[dir]")?.getAttribute("dir") === "rtl"
                    ? "rtl"
                    : "ltr");
                handles.forEach((handle) => {
                    ...
                    setAttributeIfChanged(handle, "data-orientation", handle.getAttribute("aria-orientation") ?? "vertical");
                });
            };
            ...
            const syncHandles = () => {
                ...
                handles.forEach((handle, index) => {
                    ...
                    if (before) {
                        ...
                        setAttributeIfChanged(before, "data-size", String(panelSize(before)));
                        syncHandle(handle, before);
                    }
                    if (after) {
                        if (!after.id)
