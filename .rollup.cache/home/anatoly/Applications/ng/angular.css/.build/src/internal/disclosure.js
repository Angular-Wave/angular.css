...
export function bindEscapeClose(elements, isOpen, close) {
...
export function bindOverlay(scope, element, parts) {
    ...
    const getFocusableItems = () => {
        ...
        if (!content)
            return [];
        return queryAll(content, focusableSelector).filter((item) => {
            if (!isOwned(item) ||
                isDisabled(item) ||
    ...
    const syncDirection = () => {
        ...
        const overlay = getOverlay();
        if (overlay)
            setAttributeIfChanged(overlay, "data-direction", direction);
    };
    const syncInteractiveParts = () => {
        ...
        getInternalTriggers().forEach((trigger) => {
            if (content)
                setAttributeIfChanged(trigger, "aria-controls", content.id);
