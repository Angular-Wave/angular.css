...
test("canonical input group source uses one native control", async ({
  page,
}) => {
  await page.goto("/src/patterns/input-group/input-group.html");

  await expect(page.getByLabel("Search")).toBeVisible();
  await expect(page.locator(".input-group > input")).toHaveCount(1);
});
