...
const colorSelector =
  '[data-slot="chart-bar"], [ng-chart-bar], [data-slot="chart-swatch"], [ng-chart-swatch], [data-slot="chart-tooltip-indicator"], [ng-chart-tooltip-indicator]';
const semanticSlots = {
  ...
  tooltipIndicator:
    '[data-slot="chart-tooltip-indicator"], [ng-chart-tooltip-indicator]',
...
const syncValue = (element: HTMLElement) => {
  const value = element.getAttribute("data-value");
  if (value) element.style.setProperty("--value", value);
  else element.style.removeProperty("--value");
};

const syncColor = (element: HTMLElement) => {
  const color = element.getAttribute("data-color");
  if (color) element.style.setProperty("--chart-color", color);
  else element.style.removeProperty("--chart-color");
};

const syncDirection = (element: HTMLElement) => {
  const direction =
...
const syncChartSemantics = (element: HTMLElement) => {
...
const barLabel = (element: HTMLElement): string | null => {
...
const syncBarSemantics = (
  element: HTMLElement,
  generatedLabels: WeakMap<HTMLElement, string>,
) => {
  if (!element.getAttribute("role")) element.setAttribute("role", "img");
  ...
  if (next) {
    if (current !== next) element.setAttribute("aria-label", next);
  ...
  return {
    link(scope: ng.Scope, element: HTMLElement) {
      ...
      const sync = () => {
        ...
        queryAll<HTMLElement>(element, semanticSlots.axisItem).forEach(
          (item) => {
            if (!item.hasAttribute("role"))
              item.setAttribute("role", "listitem");
          },
        );
        queryAll<HTMLElement>(element, semanticSlots.grid).forEach((grid) => {
          if (!grid.hasAttribute("aria-hidden")) {
            grid.setAttribute("aria-hidden", "true");
          }
        });
        queryAll<HTMLElement>(element, semanticSlots.legend).forEach(
          (legend) => {
            if (!legend.hasAttribute("role"))
              legend.setAttribute("role", "list");
          },
        );
        queryAll<HTMLElement>(element, semanticSlots.legendItem).forEach(
          (item) => {
            if (!item.hasAttribute("role"))
              item.setAttribute("role", "listitem");
          },
        );
        queryAll<HTMLElement>(element, semanticSlots.tooltip).forEach(
          (tooltip) => {
        ...
        queryAll<HTMLElement>(element, semanticSlots.tooltipItems).forEach(
          (items) => {
            if (!items.hasAttribute("role")) items.setAttribute("role", "list");
          },
        );
        queryAll<HTMLElement>(element, semanticSlots.tooltipItem).forEach(
          (item) => {
            if (!item.hasAttribute("role"))
              item.setAttribute("role", "listitem");
          },
        );
        queryAll<HTMLElement>(element, semanticSlots.tooltipIndicator).forEach(
          (indicator) => {
