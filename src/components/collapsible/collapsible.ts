  ...
  return {
    link(scope: ng.Scope, element: HTMLElement) {
      ...
      const readOpenState = () => {
        ...
        if (controlledOpen !== null) return controlledOpen === "true";
        return (
      ...
      const handleToggle = () => {
        if (details) setOpen(details.open);
      };
      const handleClick = () => {
        if (!details) setOpen(Boolean(content.hidden));
      };
      ...
      onDestroy(scope, () => {
        ...
        if (!details) trigger.removeEventListener("click", handleClick);
      });
    },
  };
}
