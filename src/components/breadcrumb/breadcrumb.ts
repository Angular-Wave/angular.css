...
const separatorSelector =
  '[data-slot="breadcrumb-separator"], [ng-breadcrumb-separator]';
const ellipsisSelector =
  '[data-slot="breadcrumb-ellipsis"], [ng-breadcrumb-ellipsis]';
...
export function breadcrumbDirective(): ng.Directive {
  return {
    link(scope: ng.Scope, element: HTMLElement) {
      const ownedCurrentPages = new WeakSet<HTMLElement>();
      ...
      const sync = () => {
        const pages = queryAll<HTMLElement>(element, pageSelector);
        const lists = queryAll<HTMLElement>(element, listSelector);
        const items = queryAll<HTMLElement>(element, itemSelector);
        const separators = queryAll<HTMLElement>(element, separatorSelector);
        const ellipses = queryAll<HTMLElement>(element, ellipsisSelector);
        const lastPage = pages.at(-1);
        const userCurrentPages = pages.filter(
          (page) =>
            page.hasAttribute("aria-current") && !ownedCurrentPages.has(page),
        );
        ...
        if (userCurrentPages.length > 0) return;
